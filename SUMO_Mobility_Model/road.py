...
class Car:
    ...
    def new_to_road(self):
        print("new")
        traci.vehicle.add(self.VehId, self.route_type, typeID="VehicleA", departSpeed=CarMaxSpeed,
...
class ICACC:
    ...
    def optimize(self, step):
        v=[]
        # print("self.new_car,step : ", self.new_car, step)
        # schedule_car = self.sa.Simulated_Annealing(self.new_car, step)
        # print("schedule_car: ", schedule_car)
        for n in self.new_car:
            v.append(n)
        for (route, veh_name) in v:
            self.road_control.assigned_car(step, Car(route, veh_name))
...
def generate_routefile():  # rou.xml 파일 만듦.
    ...
    with open("data/cross.rou.xml", "w") as routes:
        ...
        <route id="route_SN" edges="L5 L6 L15 {}" />
        <route id="route_SW" edges="L5 L6 L3 L4" />
        """.format(loaction), file=routes)
        print("</routes>", file=routes)
...
def run(car_amount):
    ...
    while True:
        if step < SimulationDuration and step % 10 == 0:
            ...
            # print("step:: intersection_management: ", step, intersection_management.new_car)
...
if __name__ == "__main__":
    ...
    run(int(car_amount))
    print("Delay time of {} vehicles/hr: ".format(car_amount))