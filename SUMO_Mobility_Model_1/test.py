...
class Car:
    ...
    def new_to_road(self):
        ...
        with open("sumo/cross.rou.xml", "r+") as f:
            lines =''
            new_line=''
            for line in f:
            ...
            f.writelines(lines)
            f.truncate()
        with open("sumo/cross.rou.xml", "a") as routes:
            print("""<vehicle id= '%s' type='VehicleA' route='%s' depart = "0" departSpeed = "5" />""" % (self.VehId,self.route_type),file=routes)
...
def run():
    ...
    intersection_management = ICACC(road_control,20)

    while True:
        if step < SimulationDuration and step % 50 == 0:
